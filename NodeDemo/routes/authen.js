...
router.post('/forgetPassword', async function(req, res, next){
  ...
  if(!user){
    // return ;//return loi
    responseData.responseReturn(res, 403, true,"Email khong ton tai");
  }
...
router.post('/resetPassword/:token', async function(req, res, next){
   ...
   try{
     ...
     user.tokenForgotExp = undefined;
     await user.save();
     responseData.responseReturn(res, 200, true,'Da reset password thanh cong');
   }catch(err){
    responseData.responseReturn(res, 400, true,'Loi Reset');
   }
   return;
})
